...
(function (global, factory) {
    ...
    var yandex = {
        needkey: true,
        fetch: ({ key: key, from: from, to: to, text: text }) => [`https://translate.yandex.net/api/v1.5/tr.json/translate?key=${key}&lang=${from}-${to}&text=${encodeURIComponent(text)}`, { method: "POST", body: "" }],
        parse: (res) =>
            res.json().then((body) => {
                if (body.code !== 200) {
    ...
    var libre = {
        ...
        fetch: ({ url: url = libreUrl, key: key, from: from, to: to, text: text }) => {
            const body = JSON.stringify({ q: text, source: from, target: to, api_key: key });
    ...
    var deepl = {
        ...
        fetch: ({ key: key, from: from, to: to, text: text }) => {
            ...
            return [`https://api${suffix}.deepl.com/v2/translate?auth_key=${key}&source_lang=${from}&target_lang=${to}&text=${text}`, { method: "POST", body: "" }];
        },
        parse: async (res) => {
            if (!res.ok) {
                if (res.status === 403) {
    ...
    const Translate = function (options = {}) {
        ...
        const defaults = { from: "en", to: "en", cache: undefined, languages: languages, engines: engines, engine: "google", keys: {} };
